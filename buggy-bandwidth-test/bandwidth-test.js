(function () {
  ...
  const MASTER_PLAYLIST_URL =
    "https://lw.bamgrid.com/2.0/hls/vod/bam/ms02/hls/dplus/bao/master_unenc_avc_aac.m3u8";
  ...
  const parseMediaPlaylist = (playlist, text) => {
    ...
    lines.forEach((line) => {
      ...
      if (currentSegment) {
        ...
        const length = parseFloat(
          line.replace(segmentTagRx, "").replace(/,$/, "")
        );
    ...
    console.log(
      `parsed playlist type:${playlist.type} id:${playlist.id} ${playlist.url}`
    );
  ...
  const loadAndParseMediaPlaylist = (playlist) => {
    ...
    return loadPlaylist(playlist.url)
      .then((text) => {
        return parseMediaPlaylist(playlist, text);
      })
      .catch((err) => {
        console.error(
          "Error while loading and parsing media playlist:",
          playlist.url
        );
        console.error(err);
      });
  ...
  const createSourceBufferController = (options) => {
    const { type, codec, mediaElement, mediaSource } = options;
    ...
    function appendSegment(segment, arrayBuffer, callback) {
      if (segment.isMap()) {
        ...
        console.log(
          `buffering segment type:${type} playlist:${segment.playlistId} start:${segment.start} end:${segment.end}`
        );
      }
      ...
      function onUpdateEnd() {
        ...
        if (segment.isMap()) {
          ...
          console.log(
            `buffered segment type:${type} playlist:${segment.playlistId} start:${segment.start} end:${segment.end}`
          );
        }
      ...
      function hasEnoughBufferSpace() {
        if (sourceBuffer.buffered.length > 0)
          // check current time, only allow 10 seconds to be buffered to ensure enough space
          return bufferEndBeforeAppend - mediaElement.currentTime < 10;
      ...
      whenReady(function () {
        ...
        const bufferHasSpace = hasEnoughBufferSpace();

        if (bufferHasSpace) {
          sourceBuffer.appendBuffer(arrayBuffer);
          sourceBuffer.addEventListener("updateend", onUpdateEnd);
        } else {
          console.warn("Waiting for buffer space");
          // wait 500 ms and try appending again
          setTimeout(() => appendSegment(segment, arrayBuffer, callback), 500);
        }
      });
    }
  ...
  const createHlsAudioStreamController = (options) => {
    ...
    function loadAndBufferSegment(segment) {
      loadSegment(segment, (xhr) => {
        sourceBufferController.appendSegment(
          segment,
          xhr.response,
          startNextSegmentLifecycle
        );
    ...
    function startNextSegmentLifecycle() {
      ...
      loadAndParseMediaPlaylist(playlist)
        .then(() => {
          ...
          if (segment) {
            ...
            console.warn(
              "No more segments to load; reached end of audio stream"
            );
            sourceEnded = true;
          }
        })
        .catch((err) => {
          console.error(
            "Error while loading and parsing a media playlist",
            playlist.url
          );
          console.error(err);
        });
      ...
      loadAndParseMediaPlaylist(playlist)
        .then(() => {
          ...
          if (segment) {
            ...
            console.warn(
              "No more segments to load; reached end of video stream"
            );
            sourceEnded = true;
    ...
    return {
      ...
      shiftQualityLevelUp() {
        ...
        if (nextPlaylistIndex < playlists.length) {
          ...
          emitter.emit("variantLevelChange", {
            variantLevel: playlists[nextPlaylistIndex],
          });
        }
      },
  ...
  const maybeCreateControllers = (args) => {
    ...
    const { mediaElement, mediaSource, variantLevels, groupedAudioRenditions } =
      args;
    ...
    if (
      stashedMediaElement &&
      stashedMediaSource &&
      stashedGroupedAudioRenditions &&
      stashedVariantLevels
    ) {
      ...
      document
        .getElementById("downgrade-button")
        .addEventListener("click", function () {
          const timeSinceLastChange = Date.now() - lastQualityLevelChange;
  ...
  const loadAndParseMasterPlaylist = (url) => {
    loadPlaylist(url).then((text) => {
      const { variantLevels, groupedAudioRenditions } = parseMasterPlaylist(
        url,
        text
      );
  ...
  emitter.on("variantLevelChange", function ({ detail }) {
    ...
    const info = `${detail.variantLevel.id} / ${megaBytesPerSecond.toFixed(
      3
    )} MB per second`;
  ...
  emitter.on("segmentLoaded", function ({ detail }) {
    ...
    const { length, bytes, latency } = segmentSamples.reduce(
      function (values, segment) {
    ...
    document.getElementById("table-segment-latency").innerText = `${Math.round(
      averageLatency
    )} milliseconds`;
    document.getElementById(
      "table-download-rate"
    ).innerText = `${averageDownloadRate.toFixed(3)} MB per second`;
  });
  ...
  emitter.on("bufferMetricsUpdated", function ({ detail }) {
    ...
    document.getElementById("table-buffer-size").innerText = `${seconds.toFixed(
      1
    )} seconds / ${megaBytes.toFixed(2)} MB`;
    document.getElementById(
      "table-playback-rate"
    ).innerText = `${averagePlaybackRate.toFixed(3)} MB per second`;
  });
